 ...
 * @author Justin C. Klein Keane <jukeane@sas.upenn.edu>
 */
class Lspgroup extends Maleable_Object implements Maleable_Object_Interface {
    // --- ASSOCIATIONS ---
	...
	/**
	 * Hosts associated with this Lspgroup.  This
    ...
    public function __construct($id = '') {
			...
			if ($id != '') {
				$sql = array(
					'SELECT * FROM lspgroup WHERE lspgroup_id = ?i',
						$id
				);
				$result = $this->db->fetch_object_array($sql);
				if (is_array($result) && isset($result[0])) {
					$this->id = $result[0]->lspgroup_id;
					$this->name = $result[0]->lspgroup_name;
					$this->email = $result[0]->lspgroup_email;
				}
			}
    }
    ...
    public function delete() {
    	if ($this->id > 0 ) {
	    	...
	    	$sql = array(
	    		'DELETE FROM lspgroup WHERE lspgroup_id = \'?i\'',
	    		$this->get_id()
	    	);
	    	$this->db->iud_sql($sql);
    	}
	...
	public function get_collection_definition($filter = '', $orderby = '') {
		$query_args = array();
		$sql = 'SELECT lspgroup_id FROM lspgroup WHERE lspgroup_id > 0';
	...
	public function get_details() {
		$retval = '<table id="lspgroup_details">' . "\n";
		$retval .= '<tr id="name"><td style="font-weight:bold;">LSP Group Name:</td><td>' . $this->get_name() . '</td></tr>' . "\n";
	...
	public function get_host_ids() {
		$sql = array(
			'SELECT host_id FROM host WHERE lspgroup_id = ?i',
			$this->id
		);
		$result = $this->db->fetch_object_array($sql);
    ...
    public function save() {if ($this->id > 0 ) {
	    	...
	    	$sql = array(
	    		'UPDATE lspgroup SET lspgroup_name = \'?s\', lspgroup_email = \'?s\' WHERE lspgroup_id = \'?i\'',
	    		$this->get_name(),
	    		$this->get_email(),
	    		$this->get_id()
	    	);
    		...
    		$sql = array(
				'INSERT INTO lspgroup SET lspgroup_name = \'?s\', lspgroup_email = \'?s\'',
    			$this->get_name(),
    			$this->get_email()
	    	);
	    	$this->db->iud_sql($sql);
    ...
    public function set_email($email) {
    	if(eregi("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$", $email)) {
			$this->email = $email;
  			...
  			$this->log->write_error('Illegal e-mail address specified, class.Lspgroup.php');
  			$this->email = '';
		}
    }
