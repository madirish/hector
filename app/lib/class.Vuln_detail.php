...
class Vuln_detail extends Maleable_Object implements Maleable_Object_Interface {
   ...
   /**
    ...
    public function __construct($id = '')
    {
		...
		if ($id != '') {
			$sql = array(
				'SELECT vd.*, v.vuln_name, v.vuln_description, v.vuln_cve, v.vuln_osvdb, vh.host_id, h.host_name FROM vuln_detail vd ' .
				'inner join vuln v on v.vuln_id = vd.vuln_id ' .
				'inner join vuln_detail_x_host vh on vh.vuln_detail_id = vd.vuln_detail_id ' . 
				'inner join host h on h.host_id = vh.host_id WHERE vd.vuln_detail_id =?i',
				$id
			);
			$result = $this->db->fetch_object_array($sql);
			$this->id = $result[0]->vuln_detail_id;
			$this->text = $result[0]->vuln_detail_text;
			$this->ignore = $result[0]->vuln_detail_ignore;
			$this->fixed = $result[0]->vuln_detail_fixed;
			$this->fixed_datetime = $result[0]->vuln_detail_fixed_datetime;
			$this->fixed_notes = $result[0]->vuln_detail_fixed_notes;
			...
			$ids = array();
			$sql = array(
				'SELECT vd.vuln_detail_id from vuln_detail vd ' .
				'inner join vuln_detail_x_host vh on vh.vuln_detail_id = vd.vuln_detail_id ' . 
				'inner join host h on h.host_id = vh.host_id ' .
				'WHERE vh.host_id=?i and vd.vuln_id=?i and vd.vuln_detail_text=\'?s\'',
				$this->host_id,
				$this->vuln_id,
				$this->text
				);
			$results = $this->db->fetch_object_array($sql);
			foreach($results as $result) $this->ids[]=$result->vuln_detail_id;
			
			$sql = array(
				'SELECT max(vd.vuln_detail_datetime) as last, min(vd.vuln_detail_datetime) as first from vuln_detail vd ' .
				'WHERE vd.vuln_detail_id in (?s)',
				implode(',', $this->ids)
				);
			$result = $this->db->fetch_object_array($sql);
	...
	public function get_collection_definition($filter = '', $orderby = '') {
		$query_args = array();
		$sql = 'SELECT vd.vuln_detail_id FROM vuln_detail vd WHERE vd.vuln_detail_id > 0';
		...
		if ($orderby != '') {
			...
			$sql .= ' ORDER BY vd.vuln_detail_id';
		}
		return $sql;
	}
    ...
    public function save() {if ($this->id > 0 ) {
	    	...
	    	$sql = array(
	    		'UPDATE vuln_detail SET vuln_detail_text = \'?s\', vuln_detail_ignore = \'?s\', vuln_detail_fixed = \'?s\', vuln_detail_fixed_datetime = \'?s\', vuln_detail_fixed_notes =\'?s\' ' .
