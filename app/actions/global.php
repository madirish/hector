...
$sql = 'SELECT COUNT(x.vuln_details_id) AS vulncount ' .
		'FROM vuln_details d, vuln_details_x_host x, host h ' .
		'WHERE x.vuln_details_id = d.vuln_details_id AND ' .
			'h.host_id = x.host_id AND ' .
			'd.vuln_details_ignore = 0 AND ' .
			'd.vuln_details_fixed = 0';
