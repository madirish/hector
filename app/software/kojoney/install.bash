#!/bin/bash
# Kojoney installer 
# Modified by Justin Klein Keane <jukeane@sas.upenn.edu>
# for use with HECTOR
...
read
more LICENSE libs/license.zope libs/twisted.license 2>/dev/null || less LICENSE libs/license.zope libs/twisted.license 
...
if [ "$license_accept" = 'yes' ]; then
	...
	echo "******************************************"
	echo "Modified by Justin C. Klein Keane <jukeane@sas.upenn.edu."
	echo "for use with HECTOR."
	echo
else
...
if [ -d $KOJONEY_PATH ]; then
	echo Directory exists. Uninstall it first using uninstall.bash
	echo Exiting...
	exit
else
	mkdir $KOJONEY_PATH
...
# Commented out by Justin Klein Keane
